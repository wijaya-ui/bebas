...
export default function StokKeluarPage() {
  ...
  const { data: stockOutHistory, isLoading: historyLoading } =
    useListStockOut();
  ...
  const filteredHistory = stockOutHistory?.filter((h) => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <h2 className="text-xl font-display font-bold mb-2">Stok Keluar</h2>
        <p className="text-muted-foreground max-w-md">
          Silakan masuk untuk mencatat stok keluar.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-display font-bold">Stok Keluar</h1>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={selectedItem} onValueChange={setSelectedItem}>
                    <SelectTrigger data-ocid="stock_out.item_select">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    min={1}
                    max={maxQuantity ? Number(maxQuantity) : undefined}
                    value={quantity}
                    onChange={(e) => setQuantity(e.target.value)}
                    placeholder="0"
                    className={isOverStock ? "border-destructive" : ""}
                    data-ocid="stock_out.quantity_input"
                  />
                  {isOverStock && (
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    data-ocid="stock_out.date_input"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    data-ocid="stock_out.notes_input"
                  />
                </div>
              </div>
              <div className="flex justify-end">
                <Button
                  type="submit"
                  disabled={recordStockOut.isPending || isOverStock}
                  data-ocid="stock_out.submit_button"
                >
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <CardTitle className="text-lg">Riwayat Stok Keluar</CardTitle>
              <CardDescription>Daftar transaksi stok keluar</CardDescription>
            </div>
            <div className="relative max-w-xs">
              ...
              <Input
                ...
                className="pl-9"
                data-ocid="stock_out.history_search"
              />
            </div>
          </div>
        </CardHeader>
                  ...
                  {filteredHistory && filteredHistory.length > 0 ? (
                    filteredHistory.map((h, idx) => {
                      ...
                      return (
                        <TableRow
                          key={h.id.toString()}
                          data-ocid={`stock_out.history.row.${idx + 1}`}
                        >
                          <TableCell>
                            {new Date(
                          ...
                          <TableCell>
                            <Badge variant="secondary" className="gap-1">
                              <ArrowUpRight className="h-3 w-3" />-{h.quantity}
                            </Badge>
                          </TableCell>
